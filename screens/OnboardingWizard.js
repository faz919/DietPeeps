...
const OnboardingWizard = ({ navigation }) => {
    ...
    return (
        ...
        >
            <MotiView from={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'timing', easing: Easing.out(Easing.ease) }} style={{ alignItems: 'center', justifyContent: 'center', paddingHorizontal: 10, flexDirection: 'row' }}>
                {Array.apply(null, { length: formLength }).map((i, index) =>
                    formPage > index ? 
                    <MotiView from={{ scale: 0 }} animate={{ scale: 1 }} style={{ borderRadius: 10, marginRight: 2, marginLeft: 2, height: 10, width: -5 + ((windowWidth - 20) / formLength), backgroundColor: '#4C44D4' }} />
                    :
                    <MotiView from={{ scale: 0 }} animate={{ scale: 1 }} style={{ borderRadius: 10, marginRight: 2, marginLeft: 2, height: 10, width: -5 + ((windowWidth - 20) / formLength), backgroundColor: '#BDB9DB' }} />
                )}
            </MotiView>
        </MotiView>}
                ...
                <View style={styles.ViewT7}>
                    {formPage === 1 &&
                        ...
                        <MotiView
                            ...
                            delay={3000}
                        >
                            <TouchableOpacity onPress={() => { setFormResponses(val => ({ ...val, weightGoal: 'Lose Weight' })); setFormPage(2) }} style={[styles.largeView, { flexDirection: 'row', marginBottom: 20 }]}>
                                <Text style={styles.title1}>Lose Weight</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => { setFormResponses(val => ({ ...val, weightGoal: 'Maintain Weight' })); setFormPage(2) }} style={[styles.largeView, { flexDirection: 'row', marginBottom: 20 }]}>
                                <Text style={styles.title1}>Maintain Weight</Text>
                            </TouchableOpacity>
                            <TouchableOpacity onPress={() => { setFormResponses(val => ({ ...val, weightGoal: 'Gain Weight' })); setFormPage(2) }} style={[styles.largeView, { flexDirection: 'row', marginBottom: 20 }]}>
            ...
            {formPage > 1 &&
                <View style={{ position: 'absolute', top: Platform.OS === 'ios' ? insets.top + 20 : 20, left: 20 }}>
